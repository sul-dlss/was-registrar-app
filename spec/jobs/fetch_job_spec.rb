...
RSpec.describe FetchJob do
  ...
  describe '#perform_now' do
    before do
      ...
      expect(FileUtils).to receive(:mkdir_p).with('tmp/jobs/AIT_1/2017_11')
    end

    let(:stderr) { nil }
    context 'when the fetch is successful and there are no warcs' do
      before do
        allow(Dir).to receive(:glob).with('tmp/jobs/AIT_1/2017_11/*.warc*').and_return([])
      end
    ...
    context 'when the fetch is successful and there are warcs' do
      ...
      before do
        allow(Dir).to receive(:glob).with('tmp/jobs/AIT_1/2017_11/*.warc*').and_return(['foo.warc'])
      ...
      it 'run successfully' do
        ...
        expect(objects_client).to have_received(:register).with(params: { admin_policy: 'druid:wr005wn5739',
                                                                          ...
                                                                          object_type: 'item',
                                                                          rights: 'dark',
                                                                          source_id: 'sul:ARCHIVEIT-1-2017_11' })
      end
    end
